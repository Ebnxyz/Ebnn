...
import { ArrowLeft, Loader2, ShieldCheck, AlertOctagon, ArrowRight, Shield } from 'lucide-react';
...
export default function ContactPage() {
    ...
    if (status === 'success') {
        return (
            <div className="min-h-screen flex items-center justify-center p-4 bg-bg-dark">
                <div className="bg-aura fixed top-[-10%] left-[-10%] w-[300px] h-[300px] bg-blue-500/30 blur-[80px] -z-10 rounded-full animate-float"></div>
                <div className="bg-aura fixed bottom-[-10%] right-[-10%] w-[400px] h-[400px] bg-purple-500/30 blur-[80px] -z-10 rounded-full animate-float" style={{ animationDelay: '-5s' }}></div>

                <div className="max-w-xl w-full glass p-10 rounded-[2.5rem] shadow-2xl flex flex-col items-center text-center fade-in">
                    ...
                    <Link href="/" className="mt-8 text-slate-500 text-[10px] uppercase tracking-widest hover:text-white transition-colors">Return Home</Link>
                </div>
            </div>
        );
    }
    ...
    return (
        <div className="min-h-screen flex items-center justify-center p-4 overflow-hidden relative">
            ...
            <main className="max-w-xl w-full glass p-8 sm:p-10 rounded-[2.5rem] shadow-2xl relative overflow-hidden bg-slate-900/60 border border-white/10 backdrop-blur-xl">

                {status === 'error' ? (
                    <div className="flex flex-col items-center text-center py-10 fade-in">
                    ...
                    <>
                        <div id="headerArea">
                            ...
                            <h1 className="text-3xl sm:text-4xl font-black italic tracking-tighter text-white uppercase mb-8">
                        ...
                        <form onSubmit={handleSubmit} className="space-y-4">
                            <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
                                <div className="space-y-1">
                                    ...
                                    <input type="text" name="name" required className="w-full px-5 py-3.5 rounded-2xl outline-none transition-all bg-slate-900/80 border border-white/10 text-white focus:border-blue-500 focus:ring-1 focus:ring-blue-500/50 placeholder-slate-600" placeholder="Your Name" />
                                </div>
                                <div className="space-y-1">
                                    <label className="text-[9px] font-bold uppercase text-slate-500 tracking-widest ml-1">Protocol</label>
                                    <input type="email" name="email" required className="w-full px-5 py-3.5 rounded-2xl outline-none transition-all bg-slate-900/80 border border-white/10 text-white focus:border-blue-500 focus:ring-1 focus:ring-blue-500/50 placeholder-slate-600" placeholder="Email Address" />
                                </div>
                            </div>

                            <div className="space-y-1">
                                <label className="text-[9px] font-bold uppercase text-slate-500 tracking-widest ml-1">Subject</label>
                                <input type="text" name="subject" required className="w-full px-5 py-3.5 rounded-2xl outline-none transition-all bg-slate-900/80 border border-white/10 text-white focus:border-blue-500 focus:ring-1 focus:ring-blue-500/50 placeholder-slate-600" placeholder="Subject" />
                            </div>

                            <div className="space-y-1">
                                <label className="text-[9px] font-bold uppercase text-slate-500 tracking-widest ml-1">Payload</label>
                                <textarea name="message" required rows={3} className="w-full px-5 py-3.5 rounded-2xl outline-none transition-all resize-none bg-slate-900/80 border border-white/10 text-white focus:border-blue-500 focus:ring-1 focus:ring-blue-500/50 placeholder-slate-600" placeholder="Message content..."></textarea>
                            </div>
